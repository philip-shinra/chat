...
> **⚠️ This only works with a bind mount.** Without one, the container has only a
> *copy* of the code baked in at build time (`COPY` in the Dockerfile). Alembic
> would write the new migration file inside the container's ephemeral filesystem,
> and `--rm` deletes it the moment the command exits — the file never reaches
> your host's `alembic/versions/` folder.
>
> **The fix:** mount `./backend` into the container so the generated file lands
> directly in your project (see the `volumes:` entry on the `migrate` service in
> Step 7). Alternatives — `docker cp`-ing the file out of a non-removed container,
> or running Alembic from a host venv against an exposed Postgres port — work but
> are manual and error-prone; the bind mount is the standard local-dev pattern.
...
services:
  migrate:
    ...
    volumes:
      - ./backend:/app        # match the Dockerfile's WORKDIR — lets generated
                              # migrations land on the host (see Step 6)
    environment:
...
| Generating migrations in a container with no bind mount | The file is written to the container's ephemeral filesystem and vanishes when the container is removed |
